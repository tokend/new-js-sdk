...
export class AtomicSwapBid extends Helper {
  ...
  mustLoadById (id) {
    return Running.untilFound(async () => {
      ...
      const { data } = await api.get('/v3/atomic_swap_bids/' + id)
      return data
    })
  }
  ...
  async tryLoadById (id) {
    ...
    const { response } = await api.get('/v3/atomic_swap_bids/' + id)
    if (!response) {
      log.info('/v3/atomic_swap_bids/' + id + ' returned empty response.')
    }
  ...
  async mustLoadByBaseAsset (baseAsset) {
    return Running.untilGotReturnValue(async () => {
      ...
      const { data } = await api.get('/v3/atomic_swap_bids', { filter: { base_asset: baseAsset } })
      return data[0]
    })
  }
}
