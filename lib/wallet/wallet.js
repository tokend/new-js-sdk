...
function () {
   ...
   */
  function Wallet(email, keypair, accountId, walletId, sessionId, sessionKey) {
    ...
    if (sessionId && !(0, _lodash.isString)(sessionId)) {
      throw new Error('Hex encoded session ID expected.');
    }

    if (sessionKey && !(0, _lodash.isString)(sessionKey)) {
  ...
  (0, _createClass2.default)(Wallet, [{
    ...
    key: "fromEncrypted",
    value: function fromEncrypted(opts) {
      var rawMasterKey = crypto.calculateMasterKey(opts.salt, opts.email, opts.password, opts.kdfParams);
      var rawWalletId = crypto.deriveWalletId(rawMasterKey);
      var rawWalletKey = crypto.deriveWalletKey(rawMasterKey);
      var decryptedKeychain = JSON.parse(crypto.decryptData(opts.keychainData, rawWalletKey));
      return new Wallet(opts.email, _base.Keypair.fromSecret(decryptedKeychain.seed), opts.accountId || _base.Keypair.fromSecret(decryptedKeychain.seed).accountId(), _sjclTokend.default.codec.hex.fromBits(rawWalletId), opts.sessionId, opts.sessionKey);
    }
    /**
